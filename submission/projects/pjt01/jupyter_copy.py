...
for i in range(50):
    data_stan = datetime(2019, 7, 12) - timedelta(weeks = i)
    data = requests.get(URL + str(data_stan.strftime("%Y%m%d"))).json()
...
with open('영화인정보목록.csv', 'w', encoding='utf-8', newline='') as f:
